...
impl QuizRepository for Repository {
        ...
        BEGIN TRANSACTION;
            ...
            FOR $user_record_id IN $new_quiz_user_record_ids {
                RELATE $user_record_id -> user_quizzes:ulid() -> $quiz_record_id SET
                    ...
                    quiz_settings.active_at = type::datetime($request.active_at),
                    quiz_settings.expired_at = type::datetime($request.expired_at),
    ...
    async fn find_user_quizzes(&self, request: FindUserQuizRequest) -> Result<Vec<UserQuiz>> {
        ...
        BEGIN TRANSACTION;
            ...
            WHERE in.id = $user_record_id AND $now < quiz_settings.expired_at AND quiz_result.status IN $request.user_quiz_statuses;
        COMMIT TRANSACTION;
        "#;
        self.database
            .query(query)
